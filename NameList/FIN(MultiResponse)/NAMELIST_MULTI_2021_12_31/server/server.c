...
void Judge_AuthorityLevel(int socket, int *admin);
void cmd_O();
...
int main(int argc, char const *argv[]){
	...
	FD_SET(server_socket, &readfds_save);
	...
	while(1){
		...
		//printf("%d\n",FD_SETSIZE);
		if(n<=0){
			write(2,"ERROR_select\n\0",Char_Num_Count("ERROR_select\n\0"));
		}
		if(FD_ISSET(server_socket, &readfds)){
			...
			//権限判断
			Judge_AuthorityLevel(client_socket, admin);
			if(*admin!=-1){
				adminCount = 1;
			}
			FD_SET(client_socket, &readfds_save);
		...
		for(i=0;i<maxfd+1;i++){
			...
			if(FD_ISSET(i, &readfds)>0){
				...
				if( (strLen = recv(i, receivebuffer, MAX_LINE_LEN, 0) ) == -1 ){
					...
					if(i == *admin){
						AuthorityLevel = 1;
						IsServerPreserveData = tmpServerPreserveData; 
						IsCmdRServerFile = tmpCmdRServer;
						cmd_O();
					}else{
						if(adminCount != 0){
							AuthorityLevel = 0;
...
void Judge_AuthorityLevel(int socket, int *admin){
	...
	if(strcmp(receivebuffer, "ADMIN") == 0 && *admin == -1){
...
void cmd_overwrite(int socket){
	...
	if(AuthorityLevel == 1){
		...
		printf("%%Oは管理者のみしか利用できません．\n");
		memset( str, 0, MAX_LINE_LEN);
		sprintf(str,"%%Oは管理者のみしか利用できません．\n");
		Send_Server2client(socket,str);
	}
}
