...
extern struct KEISOKU_TBL
    sort_keisoku_tbl[MEMBER_MAX]; /* ソート用計測データ表 */
/* 入力計測データテーブル */
...
static void keisoku_data_disp(int kaiin_code, char *keisoku_date) {
  ...
  printf("\n ** 入力計測値データ **");
  printf("\n 会員コード %3d", kaiin_code);
  printf("\n 日付 %4.4s-%2.2s-%2.2s", (keisoku_date + 0), (keisoku_date + 4),
         (keisoku_date + 6));
  printf("\n\n 運動 負荷 セット 回数");
  for (i = 0; i < 5; i++) {
    printf("\n %d %3d %3d %3d", i + 1, keisoku_indata[i].huka,
           keisoku_indata[i].set, keisoku_indata[i].kaisuu);
  }
  return;
}
/************************************/
...
static int kojin_data_update(int kaiin_code, char *keisoku_date,
  ...
  /* 計測データ表ファイル WRITE -> 1 以外 ? */
  if ((ret = fwrite((char *)&kojin_keisoku_tbl, sizeof(kojin_keisoku_tbl), 1,
                    fp)) != 1) {
...
static int keisoku_rank(void) {
  ...
  i = 0;
  for (;;) {
    /* 計測データ表ファイル READ -> 1 以外 ? */
    if ((ret = fread((char *)&sort_keisoku_tbl[i], sizeof(struct KEISOKU_TBL),
                     1, fp)) != 1) {
      /* READ エラー ? */
      if (ferror(fp) != 0) {
        printf("\n 計測データ表ファイル READ エラー");
...
static void keisoku_data_sort(int cnt) {
  ...
  for (i = 0; i < cnt - 1; i++) {
    for (j = i + 1; j < cnt; j++) {
      if (sort_keisoku_tbl[i].max_data < sort_keisoku_tbl[j].max_data) {
...
static void keisoku_sort_disp(int cnt) {
  ...
  for (i = 0; i < cnt; i++) {
    ...
    printf("\n %2d %3d %4d %4.4s-%2.2s-%2.2s", i + 1,
           sort_keisoku_tbl[i].kaiin_code, sort_keisoku_tbl[i].max_data,
           &sort_keisoku_tbl[i].max_date[0], &sort_keisoku_tbl[i].max_date[4],
